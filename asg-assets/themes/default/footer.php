<footer> &copy; 2012 - <?=date('Y');?> Copyright <a href="https://coderbits.com/stefanbc" title="Find me!" target="_blank">Stefan Cosma</a> | Powered by <a href="https://github.com/stefanbc/Asgar" title="Asgar" target="_blank">Asgar</a></footer>

<script src="<?=asg_themefolder('js',false)?>jquery.sharrre-1.3.4.min.js"></script>
<script src="<?=asg_themefolder('js',false)?>jquery.zclip.min.js"></script>
<script src="<?=asg_themefolder('',false)?>custom.js"></script>

<?=asg_scripts('footer')?>

</body>
</html>